...
import static com.spookedengine.scenegraph.Trimesh.*;
 ...
 * @author Oliver Winks
 */
public class AndroidOBJLoader extends ModelLoader {
    ...
    public AndroidOBJLoader(Activity context) {
    ...
    @Override
    public Visual loadModel(ResourceScope scope, String filename) throws Exception {
        logger.log(Level.INFO, "loading model " + filename);
                    ...
                    if(tokSize == 0) {
                        switch(strBuf.charAt(token[0])) {
                            ...
                            case 'f':
                                ...
                                if(countTokens(strBuf, token[1] + 1, lineEnd, ' ') == 3) {
                                    ...
                                    Logger.getLogger(getClass().getName()).log(Level.SEVERE, "Object must be triangulated!");
                                }
                                break;
                            ...
                            case 'g':
                                ...
                                logger.log(Level.INFO, "Loading Group " + strBuf.substring(token[0], token[1] + 1) + "...");
                        ...
                        switch(strBuf.charAt(token[0])) {
                            ...
                            case 'm':
                                ...
                                // load material
                                logger.log(Level.INFO, "loading material " + strBuf.substring(token[0], token[1]));
                                loadMTL(scope, new BufferedReader(
                                        new InputStreamReader(context.getAssets().open(strBuf.substring(token[0], token[1] + 1)))));
    ...
    protected void loadMTL(ResourceScope scope, BufferedReader reader) throws Exception {
        Material mat = null;
        StringBuilder strBuf = new StringBuilder();
            ...
            if(strBuf.charAt(i) == '\n') {
                ...
                if(lineEnd - lineBegin > 0 && strBuf.charAt(lineBegin) != '#') {
                    ...
                    if(tokSize == 5 && strBuf.charAt(token[0]) == 'n') {
                        ...
                        textures.put(texName, ResourceManager.getInstance().loadTexture(scope, texName));
    ...
    private class Group {
        ...
        public Visual convert() {
            ...
            if(hasTexCoords && hasNormals) {
                model = Geom.new3D(name, new Trimesh(DrawMode.TRIANGLES, vertexArray, normalArray));
                model.getTrimesh().addTexCoords(texCoordArray);
            } else if (hasNormals) {
                model = Geom.new3D(name, new Trimesh(DrawMode.TRIANGLES, vertexArray, normalArray));
            } else {
                model = Geom.new3D(name, new Trimesh(DrawMode.TRIANGLES, vertexArray));
            }

            // create the material
            if (mtl != null) {
                model.getLocalAppearance().override(MATERIAL);

                com.spookedengine.scenegraph.appearance.Material newMat = new com.spookedengine.scenegraph.appearance.Material();
                ...
                if(mtl.alpha < 1) {
                    model.getLocalAppearance().override(ALPHA);
